...
if 'lead_data_df' not in st.session_state:
    st.session_state.lead_data_df = pd.DataFrame()
if 'funding_data_df' not in st.session_state:
    st.session_state.funding_data_df = pd.DataFrame()
if 'enriched_funding_data_df' not in st.session_state:
    st.session_state.enriched_funding_data_df = pd.DataFrame()
...
def scrape_lead_data(url):
    ...
    Returns partial data even if no contact info is found.
    """
    try:
        ...
        contact_type = "Email Only" if cleaned_emails and not formatted_phones \
                       else "Phone Only" if formatted_phones and not cleaned_emails \
                       else "Both" if cleaned_emails and formatted_phones \
                       else "No Contact"
...
if selected == "👤 Lead Contacts":
    ...
    # Use a button to trigger the scraping and a separate button to clear the state
    start_lead_scraping = st.button("🔎 Start Lead Contact Scraping", key="lead_button")
    clear_lead_data = st.button("🗑️ Clear Scraped Data", key="clear_lead_button")
    ...
    if start_lead_scraping:
        ...
        if not all_found_urls:
            ...
            if lead_data:
                ...
                df = df[df['Contact Type'] != "Scraping Error"]
                st.session_state.lead_data_df = df
                st.success("✅ Lead data scraped and stored successfully!")
            else:
                st.warning("⚠️ No lead data could be extracted from the scanned pages.")
    
    # This section will now run every time and use the session state data
    if not st.session_state.lead_data_df.empty:
        df = st.session_state.lead_data_df
        
        # Display stats
        st.subheader("Scraping Results Summary")
        ...
        st.write(f"No Contact Info: {len(df[df['Contact Type']=='No Contact'])}")
        
        # Filter options
        view_option = st.radio(
            "Show:",
            ["All Leads", "Email Only", "Phone Only", "Both Email & Phone", "No Contact Info"]
        )
        
        filtered_df = df.copy()
        if view_option == "Email Only":
            filtered_df = df[df['Contact Type'] == "Email Only"]
        elif view_option == "Phone Only":
            filtered_df = df[df['Contact Type'] == "Phone Only"]
        elif view_option == "Both Email & Phone":
            filtered_df = df[df['Contact Type'] == "Both"]
        elif view_option == "No Contact Info":
            filtered_df = df[df['Contact Type'] == "No Contact"]
...
# --- Company Funding Tab ---
    ...
    clear_funding_data = st.button("🗑️ Clear Scraped Data", key="clear_funding_button")

    if clear_funding_data:
        st.session_state.funding_data_df = pd.DataFrame()
        st.session_state.enriched_funding_data_df = pd.DataFrame()
    ...
    if start_company_scraping:
        ...
        if not all_funding_urls:
            ...
            if company_funding_data:
                ...
                st.session_state.funding_data_df = df_funding
                st.success("✅ Funding data scraped and stored successfully!")
                ...
                if not df_funding.empty:
                    ...
                    st.session_state.enriched_funding_data_df = df_enriched_funding
                    if not df_enriched_funding.empty:
                        st.success("✅ Data enriched with Apollo.io successfully!")
    ...
    # Display logic for the funding tab, using session state
    if not st.session_state.funding_data_df.empty:
        st.subheader(f"Found Company Funding Details ({len(st.session_state.funding_data_df)} Entries)")
        st.dataframe(st.session_state.funding_data_df)
        
        if not st.session_state.enriched_funding_data_df.empty:
            st.subheader("Enriched Company Funding Details with Comprehensive Apollo Data")
            st.dataframe(st.session_state.enriched_funding_data_df)
            
            csv_enriched = st.session_state.enriched_funding_data_df.to_csv(index=False).encode('utf-8')
